...
bug fixes have been incorporated into the code already.\Comment{
  ...
  the corresponding engines.} Overall, discarding bug reports with the
...
%% }
%% \Igor{Ha testes em que o non-standard element aparece com a mensagem de erro
%% diferente da que usamos anteriormente. Por exemplo, existem testes que 
%% estao dentro de um bloco try/catch, mesmo se a linha quebrar, 
%% o catch vai capturar e enviar uma mensagem tipo "expected true, got false".
%% }
...
unable to capture looking for error types like \CodeIn{ReferenceError}
and \CodeIn{TypeError}. For example, we found cases where the test (or
its harness) capture the corresponding exception, making the test to
fail in an assertion instead. \Mar{igor, ha alguma explicacao/intuicao
  para explicar porque nao conseguir classificar isto?.$\rightarrow$}
  \Igor{Exemplos: Testes especificos e/ou com entradas invalidas, 
  por exemplo cobre uma especificacao antiga onde o
  teste esperava uma propriedade true, mas na documentacao 
  cita que a propriedade tem que ser false.}\Mar{nao
  entendi. isto nao acabaria quebrando por um dos motivos anteriores?
  seja mais especifico, por favor!}
  \Igor{nao, o teste eh valido (nao quebra por ReferenceError/TypeError) e
  nao entra no undefined behaviour pq nao afeta a especificacao. Isso eh um caso,
  outros casos estao ligados a nao-compreensao do testcase }  \textbf{Unknown.} This category
...
%Note that bug reports \#4 and \#23 afected multiple engines. 
...
report is pending confirmation.\Mar{por favor,
...
indicates that \Mar{por favor, explique. nao consigo lembrar.}
\Igor{existem testes que realizam a comparacao de strings, o fuzzer pode
alterar o corpo da mensagem. Casos onde o fuzzer muda a msg em uma assercao 
(por exempo 'assertThrows(SyntaxError(), "SyntaxError: syntax error")').
}
...
\Fix{ten} of the bugs we reported were confirmed, \Fix{two} of which
...
\begin{center}
  \fbox{
    \begin{minipage}{8cm}
      \textit{Summary:}~Cross-engine differential testing was also
      effective at finding bugs in \js\ engines. However, compared to
      test transplantation, it required substantial more human effort
      per bug found.
      \end{minipage}
    }
\end{center}
...
\sloppy
\vspace{1ex}\noindent\textbf{Issue \#4, Table~\Fix{XX}.} The code
snippet below shows the test input we used to reveal a bug in
\chakra.
...
The object property \CodeIn{valueOf} stores a function
that returns a primitive value identifying the target
object~\cite{valueof}. The original version of this code returns an
empty string whereas the version of the code modified by the \radamsa{}
fuzzer~\cite{radamsa} returns a string representation of a null
character (\CodeIn{NUL} in ascii). The unary plus expression
...
Number)\cite{unary-plus}. For this case, \chakra{}\Mar{\chakra? if
  chakra did that it would be fine, no?} \Igor{
  \chakra{} converte incorretamente o valor hexadecimal para
  0 e a expressao +0 resulta em 0, o que faz com que o teste passe. Os outros
  engines consideram o hexadecimal invalido (valor esperado), 
  quando o hexadecimal nao pode ser convertido, espera-se que retorne NaN, que +NaN resulta em NaN.
  }evaluates the unary
...
\vspace{1ex}\noindent\textbf{Issue \#18, Table~\ref{tab:bugs}.}
\Igor{
...
\begin{figure}[h!]
  ...
  \lstset{escapeinside={@}{@},
    ...
    basicstyle=\ttfamily\scriptsize, boxpos=c,
    numberstyle=\tiny,
    morekeywords={assert, assertEq, var, yield, in, function, eval,
    typeof, return, throw, new, Error, if},
  }
  \begin{lstlisting}
eval(function b (a) {
...
passed as argument to the \CodeIn{eval} function, encoded the actual
test. The fuzzer replaced the string argument with a function whose
body is a \CodeIn{break} statement outside a valid block
...
argument to \CodeIn{"foo".repeat()}, the original test used the
