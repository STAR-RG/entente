...
which are part of the application. We found that different engines often
...
function input compared to another engine and that is acceptable. For
instance, it is acceptable to pass values greater than \Fix{x} to
function \CodeIn{\Fix{WWW}} in \Fix{y} but not in \Fix{z}. In
...
found with fuzzing originated from ``\lo{}'' warnings. \Fix{quickly
  explain why LO can still be a bug.}
...
\begin{figure}[!h]
  ...
  \begin{lstlisting}
    ...
    #### Chakra
    RangeError: DataView operation access beyond specified buffer length
    #### SpiderMonkey
    RangeError: invalid or out-of-range index
    #### JavaScriptCore
    RangeError: byteOffset cannot be negative
  \end{lstlisting}
  \caption{\label{fig:lo-truepositive}True positive example in LO warnings.}
\end{figure}
