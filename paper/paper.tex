...
Along the year, a new version of EcmaScript (the latest version is ES2017) adds
...
For example, the latest version of ES specs~\cite{es6-spec} describes that,
the function \CodeIn{Number.toPrecision(precision)}, in general,
returns a String containing the Number value represented either 
in decimal exponential notation or in decimal fixed notation, 
if precision is defined~\cite{es6-toPrecision}. The Number object uses a 
property called
...
\end{lstlisting}
\normalsize
\caption{Warning captured as LO priority.}
...
XS~\Fix{cite}. As the project is young, \Fix{created in xx.yy.zz}), we
...
from the community\Fix{is this true?}. It is worth noting that we used
...
\begin{table}[t]
  ...
  \begin{tabular}{ccr}
    ...
    Name & URL & \# Stars \\
    \midrule
    MS's Chakra & \cite{chakra2018repo} & - \\
    Google's v8 & \cite{v82018repo} & - \\
...
tests in all engines.\Mar{not entirely clear what you did to support
  this.$\rightarrow$}\Mar{apenas explique o que fez (mesmo
...
%  \scriptsize
  ...
  \begin{tabular}{cccccccc}
    ...
    \midrule    
    1  & 4/12 & radamsa & Chakra   & \textbf{Fixed}  & \href{https://github.com/Microsoft/ChakraCore/issues/4978}{\#4978} & HI & WebKit \\ 
