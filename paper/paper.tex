...
acceptable. For instance, it is acceptable to pass values greater than
...
code not the engine (library or runtime) code. It is worth noting that
``\lo'' warnings can be bug
revealing. Figure~\ref{fig:lo-truepositive} shows one of these
...
violates the spec. This bug was fixed by developers after our bug
report and is no longer present in the most recent release of
\chakra. Despite that counter-example, most bugs we found emerged from
``\hi{}'' warnings.
