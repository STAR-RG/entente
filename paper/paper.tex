...
alterar o corpo da mensagem. Esta era a ideia, mas 
casos onde o fuzzer muda a msg em uma assercao 
(por exempo 'assertThrows(SyntaxError(), "SyntaxError: syntax error")') 
todos os engenhos falham e o teste nao ser reportado caso alcance esse bloco.
Porem, existem casos em que o teste falha por message mismatch, 
mas a discrepancia ocorre no engenho que nao exibe essa mensagem. 
Analisando os casos que encontramos dessa natureza, 
a comparacao da mensagem aparece no report, 
porem a discrepancia ocorre quando o teste nao mostra 
essa mensagem (passa por motivos de invalid input).
Eu classifiquei como message mismatch, 
mas creio que nesse caso seria invalid input.
}
...
Overall, we reported \Fix{XX} bugs in this study. Of which, \Fix{YY}
were confirmed and \Fix{ZZ} were fixed by developers. Although more
work is necessary to reduce cost of manual analysis, we found that
both techniques we used are effective at finding bugs in the engines
used in the most popular we browsers used
today. Section~\ref{sec:lessons} elaborates on the lessons we learned
in this study. The result data, the infrastructure, and experimental
scripts we prepared to run this study are available upon request.
