...
and bugs are available on Zenodo~\footnote{https://zenodo.org/record/4116092} and online at the following link: \dataRepo.
...
approach would work on a recently introduced JS engine. More about
...
\vspace{0.2cm}
\noindent
\textbf{Cleansing}~We noticed that some of the tests we found
...
\vspace{0.2cm}
\noindent
\textbf{Test Harness}~We noticed that some engines use a
...
\vspace{0.2cm}
\noindent
\textbf{Dedup}~The number of tests in \veight\ is low because
we discarded duplicate tests with Mozilla and \jsc. The rationale is to
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{figs/diff-testing-runtimes-2.pdf}
  \caption{\label{fig:workflow}The workflow of the referred differential
testing approach. In this Activity Diagram, the first black clircle is the start of the process, while the last one if the termination of the process. The green boxes mean activites to be done during the differential testing approach. The small black box mean that there are parallel activities (which in the end are combined).}
\end{figure}
...
we obtained while analyzing bugs. The types ``\hi{}'' and
...
\vspace{0.2cm}
\noindent
\textbf{Exploratory Phase} For the first
...
\vspace{0.2cm}
\noindent
\textbf{Non-Exploratory Phase} Results
...
features not yet supported by \hermes\footnote{See
...
string with three repetitions of the string ``foo''. The new test uses
...
results might not generalize to other test inputs and engines.
We though carefully selected inputs from various
sources according to a well-defined criteria (Section~\ref{sec:seeds}).
Likewise, we selected
the engines by using using a documented criteria (Section~\ref{sec:methodology:engines})
and found that the
...
inadvertently lead to interference in the well-specified parts of the
