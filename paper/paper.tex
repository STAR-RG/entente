...
This section reports the failures observed when running a test suites
on an engine for which those tests were not run
before. Table~\ref{tab:cross-testing} shows the number of failures for
each pair of test suite and engine. The first column shows the test
suites whereas the first row shows the engines. We use a dash (``-'')
to indicate that we did not consider the combinations that associated
a test suite with the engine that originated them; failures in those
cases would either indicate regressions or flaky tests as opposed to
an unknown bug for that engine. We also discarded such test cases from
consideration in other engines as we wanted the test to pass on its
originating engine; a failure in that case would be a stronger
indicative of a bug.

To sum, we observed that, considering this dataset, the Mozilla
...
\begin{itemize}
  \item \textbf{Undefined.} Implementation-dependent behavior as per
    ...
    \Igor{a test from JerryScript suite uses a function \CodeIn{Number.toPrecision([precision])}
    that returns a string representing the number object in
    fixed-point or exponential notation rounded to precision. According to ES specifications(cite)
    built-ins that involves Math functions are implementation-dependent 
  ...
  \item \textbf{Future Feature.} Test fails because it uses a feature
    ...
    \Fix{XXX} \Igor{Creio que neste experimento, essa funcao nao vai 
    aparecer. Identificamos isto em alguns testes da suite test262(nao estamos utilizando ela aqui).}
