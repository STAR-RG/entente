...
find javascript engines projects defined by the keyword
``javascript-engine''\Mar{is this a label or two keywords?}.  The
resulting list included a total of 33 projects.  A total of \Fix{4?}
projects remained after removing projects that \Fix{...}.\Mar{explique
como o numero baixou tanto.}
...
\Fix{quick on mutational fuzzers}

Coverage-based fuzzers like American Fuzz Loop (AFL)~\cite{afl} and
libfuzzer~\cite{libfuzzer} run tests inputs in coverage-instrumented
versions of the program under testing. The typical approach to input
generation of this kind of fuzzer is evolutionary--they try to
minimize the distances to still uncovered branches of the
program. AFL, for instance, takes the instrumented program binary and
one input seed and produces on output crash inputs. Considering our
context of application, we needed to build one runtime engine to
collect coverage information for fuzzing; we chose v8. (There are
options in LLVM clang for that~\cite{libfuzzer}.)  Unfortunately, we
found that most of the inputs produced violate the JS grammar and the
fuzzing task can take days even when considering one seed. That
happens because the entire decision tree of the engine is explored,
including the branches associated with the lexer and parser. For this
reason, Google promotes the creation of fuzz targets associated with
specific program functions. Although that approach has shown very
effective, it requires domain knowledge to create the calling context
for the fuzz target. For those reason, coverage-based fuzzers were not
