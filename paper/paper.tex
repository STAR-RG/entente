...
is worth noting that we mined the repositories of JS engines and found
a total of \Fix{XX} JS files associated with \Fix{YY} test suite.\Mar{show table with name and count of JS files}

The bug-finding process works as follows.  Considering the case of a
single input file, the oracle checks whether or not the output
...
inspection. Considering the case fuzzing~\cite{fuzz-testing-history}
...
differently, leading to premature failures. Although we did find real bugs from warnings manifested
in this category, the proportion is much lower compared to the HI
category--only 12\% of the reals bugs we found originated from the LO
category. \Mar{show real examples of HI and LO and provide the
  rationale for such classification.} \Mar{can we detect dups mining issue trackers?}
...
This section shows results obtained under our
