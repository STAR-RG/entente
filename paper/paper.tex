...
%\titlerunning{Leveraging Diversity to Find Bugs in JavaScript Engines}        % if too long for running head

\author{Jefferson Silva \and Igor Lima \and Breno Miranda\footnotemark
...
significant fraction (5 to 15\%) of the tests in that stuite fail
...
%  \vspace{-2ex}
  ...
  \caption{\label{fig:summary}Summary of bug reports.\Gustavo{Não esquecer de atualizar as figuras.
  }}
  \vspace{-3ex}
\end{figure}

\sloppy \emph{Results.}~Our study considered the following
engines--\chakra{}~(Microsoft), \hermes~(Facebook),
JavaScriptCore~(Apple), \smonkey{}~(Mozilla), and
\veight{}~(Google). Figure~\ref{fig:summary} shows the breakdown of
bug reports per engine (\ref{fig:stacked-engine}) and per technique
(\ref{fig:stacked-technique}).  \Fix{Jefferson, atualize a figura!
  Estranho nao ter pilha para as 5 engines.}  Each stacked bar breaks
down the bugs per status (\eg{}, ``1-New''). The number indicates the
ordering that status labels are assigned by developers: ``New'', then
``Confirmed'', then ``Fixed''. Several of these reports have the label
``3-Fixed'', indicating that bug fixes have been incorporated into the
code already. Note that most of these bugs affected two
engines--\chakra{} and JavaScriptCore (\jsc{}).  We reported
...
Transplation are practical and effective to find bugs on real,
complex, and widely used pieces of software, 2)~Even for problems with
fairly clear specifications, as in \javascript{}, there is likely (a
lot of) variation between different
implementations. Section~\ref{sec:lessons} expands and elaborates key
...
effectiveness of two practical techniques in revealing functional bugs
in popular \javascript\ engines. The results of the study include:
1)~A number of bugs found and fixed and 2)~An infrastructure for
performing test transplantation and differential testing. Our data and
tools are publicly available under the following link:
...
is practical to find functional bugs in JavaScript engines and should
be encouraged.
...
%%  LocalWords:  DataView setInt Radamsa QuickFuzz Grammarinator
%%  LocalWords:  LangFuzz Megadeth libFuzzer sanitizers
