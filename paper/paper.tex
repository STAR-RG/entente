...
``compile-in-all'' and ``no-fail-in-all'' correspond to the test files
used, respectively, in the test transplantaion experiment and the
differential testing experiment.

Column ``compile-in-all'' shows the number of test cases whose
execution does not throw errors because of an undefined variable or
property. We capture these cases by looking for the presence of
...
spec, not all engines currently support. We used the tests that lie
within the circle that appears in the table in the evaluation of test
transplantion, \ie{} the set ``compile-in-all'' minus set Test262. We
did not consider tests from the conformance suite as they are more
likely to indicate missing features as opposed to bugs. In addition,
engine developers have access to these test and are encouraged to run
...
\begin{table}[t]
  ...
  \begin{tabular}{ccrrr}
    ...
    29,846 & 17,639 \\
    \midrule
    \jsc{} & \cite{webkit} & 1,031 &\marktopleft{c1} 1,021 & 955\\
    ...
    Tiny-js & \cite{tinyjs} & 49 & 37\markbottomright{c1} & 37\\
...
cases that fail on its parent engine in any other engine as that would indicate that
the test case could be broken. In total, 64 test cases
