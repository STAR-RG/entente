...
hold the status ``new'', which indicates that they were not reviewed
yet by the development team. Note also that we did not find bugs in
the Mozilla \smonkey\ engine and that several of the bugs we reported
were already fixed and incorporated in the latest release of
corresponding engines. Overall, discarding bug report with the ``new''
label, we reported \noBugsTotalConfirmed{} bugs.
...
\begin{wraptable}[12]{r}{0.25\textwidth}  
  ...
  \caption{\label{tab:test262}Percentage of passing tests on
    the Test262 conformance suite.\Mar{Please, use percentages.}}
  \small
  \begin{tabular}{crr}
    \toprule
    engine & \% passing \\
...
in Test262 (as per Table~\ref{tab:test262}) and reliability as
measured by the number of bugs found (as per
Figure~\ref{fig:summary}), we do not not expect causality. We
conjecture that the development age of the engine--Chakra is the
newest engine\Fix{find DOB of Chakra}--justify the incidental
correlation.

%% The rationale is that lack of support
%% to features (as measured by conformance testing) and reliability of
%% engines (as measured in this study) may have some relationship. 

\subsection{Test Transplantation}
\label{sec:transplantation}

This section reports results of test transplantation. We analyzed the
failures observed when running a test suite original from a given
engine in another engine. Intuitively, we want to assess how promising
is to use knowledge of developers of a given engine to find bugs in a
different engine.
...
(7.72\% of the files). A developer with experience in \js\ analyzed
each of these files in separate as to classify if the failure was
fault-revealing or not (\ie{}, a false positive).  We observed from
...
distribution of false and true positives in this experiment. The
piechart at the top shows false alarms. The absolute number of tests
in each category is shown in parentheses to the right-side of each
slice label, which names the source of false positive. The sources of
...
\begin{itemize}
  ...
  \item \textbf{Timeout/OME\footnote{ome is for out of memory
    ...
    test may fail to finish at the specified time budget or it exceeds
    the memory budget. For example, a test case from the \jsc{} suite
    defines a function with a tail-call recursion. The test fails in
    all engines but \jsc{}, which implements tail-call optimization,
    which enables the test to finish execution fast.
  ...
  \item \textbf{Not implemented.} False positives of this kind
    ...
    engine yet. For example, at the time of writing, \chakra{} does
    not implement by default various properties from the
    \mcodeid{Symbol} object. These properties are only available
    activating the ES6 experimental mode with the flag
    \CodeIn{-ES6Experimental}.

  \item \textbf{Unknown.} This category includes the cases we were
    unable to confidently diagnose the failure but we suspected it was
    a false alarm.

\end{itemize}  

We observed that ``Unknown'' and ``Undefined'' were the most
predominant sources of false positives. Considering true positives, we
found a reasonable number of duplicate reports, but not enough high to
justify attempting to automate the detection of duplicates. 
...
%  \scriptsize
  ...
  \begin{tabular}{cccccccc}
   ...
   \midrule
   5 & 5/02  & \jsc{} & New  & \anonym{\href{https://bugs.webkit.org/show\_bug.cgi?id=185208}{\#185208}} & - & \smonkey{} \\
   ...
   11 & 7/18  & \jsc{} & New & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=187777}{\#187777}} & - & \jerry{}\\
   ...
   14 & 8/07  & \jsc{} & \textbf{Fixed} & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=188378}{\#188378}} & 2 & \jerry{}\\
   15 & 8/07  & \chakra{} & New & \anonym{\href{https://github.com/Microsoft/\chakra{}Core/issues/5579}{\#5579}} & - & \jerry{}\\
   16 & 8/07  & \jsc{} & \textbf{Fixed} & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=188382}{\#188382}} & 2 & \jerry{}\\
   17 & 8/08  & \veight{} & \textbf{Fixed} & \anonym{\href{https://bugs.chromium.org/p/v8/issues/detail?id=8033}{\#8033}} & 3 & \jerry{}\\
   18 & 8/08  & \jsc{} & New & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=188407}{\#188407}} & - & \jerry{}\\
   19 & 8/16  & \veight{} & WontFix & \anonym{\href{https://bugs.chromium.org/p/v8/issues/detail?id=8064}{\#8064}} & 2 & Duktape\\
   20 & 8/22  & \chakra{} & \textbf{Confirmed} & \anonym{\href{https://github.com/Microsoft/\chakra{}Core/issues/5621}{\#5621}} & 2 & \smonkey{}\\
   21 & 8/22  & \jsc{} & New & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=188874}{\#188874}} & - & \smonkey{}\\
   22 & 8/22  & \jsc{} & New &  & - & \smonkey{}\\
   \midrule
    \multirow{3}{*}{23}  & 
    \multirow{3}{*}{8/22} & \jsc{} & New & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=188875}{\#188875}} & - & \multirow{3}{*}{\smonkey{}}\\
    &  &
    \veight{} & New & \anonym{\href{https://bugs.chromium.org/p/v8/issues/detail?id=8082}{\#8082} } &  -  & \\
    &  &
    \chakra{} & New & \anonym{\href{https://github.com/Microsoft/ChakraCore/issues/5624}{\#5624} } &  -  & \\
   \midrule
   24 & 8/23  & \jsc{} & New & \anonym{\href{https://bugs.webkit.org/show_bug.cgi?id=188877}{\#188877}} & - & \smonkey{}\\
   \bottomrule
  \end{tabular}
\end{table}
...
''Fixed''), column ``Url'' shows the web address for the
bug report entry in the bug tracking system (de-anonymized upon request), column
...
Considering severity levels, it is important to note that \jsc{} uses
five levels~\cite{jsc-severity} whereas
...
\Fix{8} are severity-2 bugs.  \chakra{} was the engine with the
highest number of confirmed bugs, \Fix{8} in total, with \Fix{1} bug
...
some pending reports from April 2018. Although we did not find any
showstopper critical bugs, most of the bugs are seemingly important as
per the categorization given by engineers. Analyzing the issue tracker
of \chakra, we found that severity-1 bugs are rare.
...
account the cost of inspecting the warning~\cite{harman-ohearn-2018};
it took a week for the developer to inspect the set of warnings
entirely. The precision for \jsc\ was \Fix{X\%} (\ie, XX/270), which
seems a low rate, but it took approximately \Fix{x} days to find
\Fix{y} real bugs on this engine. Section~\ref{sec:bugs} show samples
...
the difference of files per bucket across fuzzers was not very
significant.
...
Table~\ref{tab:summary-hi} shows statistics for \hi\ warnings; recall
...
engine, \ie, the engine manifesting a distinct output among those
...
Table~\ref{tab:cross-testing}, with \chakra\ and then \jsc\ associated
with more warnings/failures compared to other engines.
%\veight\ and \smonkey. 
...
\Mar{REVISE}So far, ten of the bugs we
...
slow for that engine.  \Igor{ The bugs reported on \chakra{}'s bug
  tracker was defined as confirmed, however the bugs are included on a
  milestone for the next release following an internal priority.  } As
...
noting that \Fix{11 of the 19} JS files that manifested discrepancies
