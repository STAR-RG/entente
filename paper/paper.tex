...
testing.  We analyzed the impact of two orthogonal dimensions in this
study: test mining and fuzzing. The first dimension focuses on the
impact of inputs, mined from the web, in bug finding whereas the
second dimension focuses on the impact of fuzzing these inputs.

In the following, we describe the methodology we used in this
experiment. Recall that our tool outputs a set of buckets, that each
...
potential duplicates. If none was reported, we filed a bug report in
the bug tracker including the test input that manifested the potential
problem and explaining the reasons we were suspicious. In some cases,
we used the Mozila lithium tool~\cite{lithium} to minimize the test
case. For the cases of false positives, we classified the reason for
the misclassification and moved to the next
bucket. Table~\ref{tab:summary-of-results} summarizes our results. It
shows, for each execution mode and category of bucket (\ie{}, hi or
lo), the total number of files involved, the number of buckets
reported, and the percentage of those that resulted in a bug
report. \Fix{elaborate summary} We elaborate those results in the
following.
...
\begin{table}[t]
  ...
  \begin{tabular}{ccrrr}
    \toprule
    \multirow{2}{*}{mode} & \multirow{2}{*}{type} & \multirow{2}{*}{\# files} &  \multicolumn{2}{c}{buckets} \\
    \cline{4-5}    
    & & & \multicolumn{1}{c}{\#} & \multicolumn{1}{c}{\% positive} \\
    \midrule
    \multirow{2}{*}{no-fuzz} & lo & \Fix{.} & \Fix{.} & \Fix{.} \\
    & hi & \Fix{.} & \Fix{.} & \Fix{.} \\
    \multirow{2}{*}{\radamsa} & lo & \Fix{.} & \Fix{.} & \Fix{.} \\
    & hi & \Fix{.} & \Fix{.} & \Fix{.} \\
    \multirow{2}{*}{\quickfuzz} & lo & \Fix{.} & \Fix{.} & \Fix{.} \\
                             & hi & \Fix{.} & \Fix{.} & \Fix{.} \\    
    \bottomrule     
  \end{tabular}
\end{table}
...
\subsection{No Fuzzing}

This section reports the results obtained without fuzzing. The goal is
to observe the impact of the mined test cases in revealing
differential bugs. In this experiment, we ran all tests (see
Section~\ref{sec:seeds}) using our infrastructure in no-fuzzing
mode. 

\Igor{
  Apos este experimento, foram obtidos \nofuzzAll{} buckets sendo destes \nofuzzHITotal
...
noise, we did not consider the \Fix{XX} files (\Fix{2.8\%} of the
