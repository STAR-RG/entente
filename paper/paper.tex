...
bugs in JS engines is therefore an important problem. It is also
challenging because of the speed the specification
evolves~\cite{ecmas262-spec-repo} and the fact that the behavior of
various language elements is undefined~\cite{so-undefined-behavior}
(\ie{} left for the implementation to define).
...
\emph{test transplantation}. This method evaluates the effects that
running \js{} test files from a given engine have in detecting bugs in
other engines. The intuition is that when a bug is found in some
engine a test case is created and added to the regression suite. Those
test inputs often check interesting scenarios that could reveal bugs
in different engine implementations. The second method considered is
\emph{cross-engine differential testing}. This method fuzzes existing
inputs~\cite{fuzz-testing-history} and then compares the output
produced by different engines using a differential oracle. One
possible cause of output discrepancy is a bug. It is worth noting that
...
popularized, certainly because it is more challenging to fully
automate--a number of legitimate reasons exist, other than a bug, for
a test execution to manifest discrepancy (see
...
It is worth noting that undefined behavior makes differential testing
more challenging.
...
optimizations. For example, the \mcodeid{for-in} loop does not clearly
...
%% fixed notation. The Number object uses a 
