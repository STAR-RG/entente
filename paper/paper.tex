...
\item \emph{Cross-engine differential testing}.
  This technique fuzzes existing test
  inputs~\cite{fuzz-testing-history} and then compares the output
  produced by different engines using a differential
...
%
\emph{Related Ideas.}~Differential Testing~\cite{Brumley-etal-ss07} (DT)
...
~\ref{tab:false-positives}). Variations of these ideas have
...
%  \vspace{-2ex}
  \centering
  \subfloat[\label{fig:stacked-engine}Bug reports per engine.\Gustavo{Não esquecer de atualizar as figuras.
  }]{
