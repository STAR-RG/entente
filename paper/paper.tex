...
new features and minor fixes for the language. \Fix{discutir mais sobre isso}
...
libraries.
\Mar{what is that exactly? can we cite? is that only spec or
actually code as most of java.* libraries.}
\Igor{seems like an API. They define the language behavior; grammar;
values to static variables; the methods for each "class"(Object)...
These irregularities appears on engine implementation, 
maybe an wrong if statement and so on.}
We found that different engines often check pre-conditions of those
...
could be considered as a case of incompatibility by engine design.}
...
XS~\Fix{cite}. As the project is young, created in Oct 2017, we
...
\Igor{eu pensei sobre isso, acho que nao eh relevante colocar o XS sendo 
que ele eh uma engine para iot/embarcados, temos como exemplo o 
JerryScript que tambem tem o mesmo proposito e esta ha bem mais tempo
que ele. Por mim, utilizamos apenas o Jerry ou os dois (e compara-los ja que 
sao similares), oq vc acha?}. It is worth noting that we used
...
\Igor{sim, poderiamos fazer isso. Porem, lembro que alguns testcases por exemplo usam 
algo diferente de "assert", outras usam alguma funcao interna 
que apenas uma engine suporta. Eu posso analisar melhor aqui nossas seeds 
e ver um jeito global de resolver isso, como se fosse um "shell.js" da nossa 
ferramenta.}
...
\Igor{
  After ran all our mined seeds (see \ref{sec:seeds}) without fuzzing we found 
  a total of \Fix{arg}
