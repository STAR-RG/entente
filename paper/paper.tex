...
\begin{table}[t]
  ...
  \begin{tabular}{ccrrrr}
    ...
    BlogEngine.NET & \cite{babel2020repo} & \blogEngineNetOriginal{} & \blogEngineNetPassInPar{} & \blogEngineNetCompileAll{} & \blogEngineNetNoFailAll{} \\
    \midrule
...
``no-fail-in-all'' column), it would significantly \Mar{Not sure if I
  understood the conclusion. Do you mean the total amount of test
  (also from other suites)? Why?}reduce our amount of test files.
...
we found were based on Node.js~\cite{node} which\Mar{@Igor, is this
  the reason?  Please, confirm$\rightarrow$}\Igor{yes, it is. Some Node internal features are 
not included in the JS engine, including some keywords (ie. require, module).
In addiction, a project with nodejs require a package.json with a lot of external dependencies.} require libraries to be
...
credit where it is due\Mar{@Gustavo/@Breno, we can mention in the
  author response that we did sanity/diligence check.}.
...
specific assertions, \Igor{for example, tests provided by Mozilla contains a lot 
of custom functions (\ie\ assertThrowsInstanceOf, assertEqArray, getPromiseResult).
The shell contains those functions to made the test run correctly.}
\Mar{@Igor, can you name which?}. For that, we
...
file work in the other engine.\Mar{This seems a different problem
  (tests without assertions). I did not understand how this is related
  to portability?}\Igor{A test is a simple JS file input.
  Some tests are not well-defined.}  For instance, if a test had a statement such as
...
javascript binaries itself, in this case the tool captures all output provided by the binaries}.
...
\Gustavo{Falar pq Hermes nao esta na tabela}
...
Android''~\cite{hermes2020repo}.\Mar{short sentence saying where it is
  used.}\Igor{Currently, hermes is being used in the react-native framework,
  also maintained by Facebook, as a beta component to increase the performance of Android applications,
  such as startup time, reduce memory consumption and application size\cite{reactnative2020hermes}.
}
...
framework, but \hermes was unable to interpret those assertions \Igor{due to 
the missing features mentioned before (\ie\ Class definitions, Proxy properties)}
...
the other engines, though.\Mar{Why? Was the framework using some of the
  features aforementioned? Say it, please}\Igor{fixed above}.
Overall, we reported three issues to the \hermes bug tracker as result of using test
