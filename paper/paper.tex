...
\CodeIn{b+/v/a} represents a bug confirmed in \hermes\ engine with 
...
This is a case of validation of RegExp flags. The fuzzer mutate
the file with a regular expression "/v/" between the plus operator and
the variable. The plus operator in this case is used as a flag of the RegExp "+/v/",
but in this case the plus operator is not valid. The behavior expected
is throws an early SyntaxError due to a invalid regular expression flag,
but \hermes Hermes seems not treats the early validation of the regexp flags
