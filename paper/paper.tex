...
used to find projects with JS files\Mar{why the query is so specific?
  as per ``q=javascript-engine''? Need to be consistent to what you
  are saying an coherent to what you actually did.}. The query sorts
results in descending order in the number of stars\Mar{sorting is only
  relevant if you set a limit on the number of projects you will
  analyze. What limit is that?}. The resulting list includes a total
of \Fix{XX} projects. A total of \Fix{...}  projects remained after
