...
Android''~\cite{hermes2020repo}.\Mar{short sentence saying where it is
  used.}
...
the other engines, though\Mar{Why? Was the framework using some of the
  features aforementioned? Say it, please.}. Overall, we repoted three
...
developed engines. \Mar{check $\rightarrow$}The issue diagnosis of
...
revealed a bug in \veight{} version 7.0.244. This code snippet was
...
shows the test input we used to reveal a bug in \jsc. 
...
reported the code snippet below to the \chakra\ development team as a
...
reported the code snippet below to the \smonkey\ development team as a
bug, the bug was confirmed in a few hours.
...
This is a test case original from the \hermes suite using test transplantation.
According the specification, in strict mode, the engine must use 
the prefix '0o' or '0O' to represents octal numeric literals. It was an
issue which explores a deprecated octal token after ASI (Automatic Semicolon Insertion).
The others engine throws a SyntaxError due the missing of the octal token, but 
\smonkey returns a integer 8 that represents 010 in octal literal.


\vspace{1ex}\noindent\textbf{Issue \#24, Table~\ref{tab:bugs}.} The snippet 
above represents a bug confirmed in \hermes\ engine with 
...
This is a case of validation of RegExp flags because the fuzzer mutated
the file with a regular expression "/v/" between the variables.
The plus operator in this case is set as a flag of the RegExp "+/v/",
but in this case the plus operator is not valid as a flag. The behavior expected
is throws an early SyntaxError due to a invalid regular expression flag,
but \hermes seems not threat the early validation of the regexp flags as 
commented by a team developer.
