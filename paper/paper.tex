...
sum, finding bugs on JS engines is an important problem.
...
input~\cite{patra2016learning}. The availability of public
implementations of JS engines (see Table~\ref{tab:engines}) and test
suites makes this bug-finding approach appealing.
...
fuzzing. The first factor, test mining, considers the effect that
\js{} test files mined from the repositories of open-source engines
have in detecting functional bugs in other engines. The second factor,
fuzzing, considers the impact in bug detection of fuzzing these files.
%% WHAT WE FOUND
...
ECMAScript (\es{}) emerged with the goal to standardize variants of
JavaScript, such as Netscape's JavaScript and Microsoft's JScript. The
\es{} specification is maintained by the TC39~\cite{tc39-github}
technical committee under the regulation of Ecma
International~\cite{es6-website}.  Every year, a new version of the
specification is delivered with new features and minor fixes. The
canonical spec today is
ES6~\cite{ecmas262-spec-repo,ecmas262-spec}. Features of these
versions and supporting engines can be found elsewhere~\cite{kangax}.
...
\subsection{Implementation-Dependent Behavior}
...
order~\cite{for-in-undefined} and different engines capitalize on
that.
...
\label{sec:undef-behavior}

Undefined behavior is not a feature of the C
language~\cite{c-undefined}.

\Mar{mention strict?}

\subsection{Unintentional Discrepancies}
...
\Igor{
  ...
  % \nofuzzWebKit{} sao do WebKit e \nofuzzGoogle{}Duktap{} para o V8 e o Duktape, 
