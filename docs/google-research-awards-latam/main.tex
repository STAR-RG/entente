...
fuzzer, instantiated to our problem. In our case, JS files are
...
invokes the oracle to check correctness. In case either all engines
consider the test output consistent (\ie{}, all pass or all fail), the
infrastructure discards the given input. Otherwise, it considers that
input as potentially fault-revealing and interesting. Currently, we
classify those alarms in two groups: HI and LO. The HI group includes
...
\begin{wraptable}[16]{tr}[0pt]{0.5\textwidth}
  \vspace{-6ex}
  \scriptsize
  \centering
...
Section~\ref{sec:design} is already functional. Table~\ref{tab:bugs}
shows the list of bugs we reported on issue trackers of different
engines in the period of a month. So far, six of the bugs we reported
...
\item Explore different strategies to reduce the number of false
  positives. For example, mining issue trackers for similar bug
  reports.
