...
engines is promising. Table~\ref{tab:bugs} shows the list of bugs we
reported on issue trackers of different engines in the period of a
month. For example, bug \# 6 was manifested by the JS code \CodeIn{var
...
\CodeIn{NUL} in ascii).
The unary plus expression ``\CodeIn{+a}", appearing in the assertion,
is equivalent to the expression \CodeIn{String.ToNumber(a.valueOf())}
that converts a string to a number, but if the string cannot be
converted, the expression should evaluate to NaN (Not a Number)
\cite{unary-plus}. For this case, all engines but Chakra evaluate the unary
