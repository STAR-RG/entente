...
example, bug \# 6 was manifested by the JS code \CodeIn{var a =
  ...
  version of the fuzzed code.}. In the original version of this code 
  \Igor{this code is the original fuzzed code, not the minimized version. we can use just (+``\textbackslash{}x00''===0)}, 
  the \CodeIn{valueOf} function returned
the empty string, whose integer representation, according to the spec of Unary Plus\cite{unary-plus},
should be zero. The modified
code returns a string representation of an invalid hexadecimal
value\Fix{igor, how do we know this is invalid?}\Igor{0x00 = NULL terminator \url{http://www.asciitable.com}, but is not a "null" keyword (+null is equal 0)}. As such, the engines
...
object, which is then converted to zero, making the test to pass. \Igor{according the technical explanation:\\
the function String::toDouble should check the length, instead of null terminator. \url{https://github.com/Microsoft/ChakraCore/commit/98fbc297e2b2781bc4d6ed3ff360dba8d18c1eda}}
\newcommand{\checkm}{Y}
