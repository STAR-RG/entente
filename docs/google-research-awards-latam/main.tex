...
rest of the warnings. \Igor{We found empirically that 88\% of the
issues reported are included in the HI group and 58\% of them was confirmed}.
...
\CodeIn{NUL} in ascii).
\Igor{The unary plus expression ``\CodeIn{+a}", appearing in the assertion,
...
(Not a Number)\cite{unary-plus}}. For this case, \Igor{isto esta correto? $\rightarrow$ all engines but} Chakra evaluate the unary
...
worth noting that \Igor{7 out of 17} JS files that manifested
