...
function basicTest() {
    ...
    if(typeof Symbol.toPrimitive !== "symbol") throw new Error('Test Failed');
    //print(Symbol.toPrimitive in Date.prototype);
    if(Symbol.toPrimitive in Date.prototype !== true) throw new Error('Test Failed');
    //print(typeof Date.prototype[Symbol.toPrimitive]);
    if(typeof Date.prototype[Symbol.toPrimitive] !== "function") throw new Error('Test Failed');
    //print(Date.prototype[Symbol.toPrimitive].length);
    if(Date.prototype[Symbol.toPrimitive].length !== 1) throw new Error('Test Failed');

    pd = Object.getOwnPropertyDescriptor(Date.prototype, Symbol.toPrimitive);
    //print(pd.writable, pd.enumerable, pd.configurable);
    if(pd.writable !== false) throw new Error('Test Failed');
...
function replaceTest() {
    ...
    if ( (+(new Date())).toString() === NaN.toString()) throw new Error('Test failed');
    if (("" + (new Date())) !== 'aiee') throw new Error('Test failed');
}
